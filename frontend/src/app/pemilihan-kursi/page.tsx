...
function rankSeatGroups(seats: Seat[], prefs: Preferences) {
  ...
  for (const [, rowSeats] of rows) {
    ...
    if (!cluster) continue;
    const score = cluster.reduce((acc, s) => acc + (s.type === "window" ? 1 : 0.8), 0); // simple avg
      ...
      <Card className="mb-4 rounded-2xl border border-zinc-200 bg-white p-4 shadow-sm">
        <div className="flex w-full flex-wrap items-center justify-between gap-3">
          <div>
            ...
            <div className="text-sm text-zinc-500">Kelas {coach.coachClass === "eksekutif" ? "Eksekutif" : "Ekonomi"} • Layout {coach.layout} • {coach.rows} baris • {capacity} kursi</div>
          </div>
